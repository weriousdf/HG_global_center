...
주소를 넣으면 버튼이 켜지고, 비워 두면 버튼이 아예 나타나지 않는다. 하나만 넣어도 된다.
...
| 히어로 (첫 화면) | 채널 버튼 하나 |
| 하단 CTA 밴드 | 채널 + 오픈채팅 |
| 오른쪽 아래 고정 | 채널 + 오픈채팅. 어느 화면에서나 보인다 (모바일은 아이콘만) |

### 1. 카카오톡 채널 주소 받기

1. [center-pf.kakao.com](https://center-pf.kakao.com) 에서 채널을 만든다
   (예전 이름 "플러스친구"). 사업자등록증이 있으면 비즈채널 인증까지 해 두면 좋다 —
   나중에 알림톡을 보낼 때 필요하다.
2. 관리자홈 → **채널 정보** → **채널 홈 URL** 을 복사한다.
   `https://pf.kakao.com/_XXXXXXX` 형태다.
3. `docs/config.js` 의 `KAKAO_CHANNEL_URL` 에 넣는다.

> 주소 뒤에 `/chat` 을 붙이면 채널 홈을 거치지 않고 **바로 대화창**이 열린다.
> 문의를 빨리 받는 게 목적이면 이쪽이 낫다:
> `https://pf.kakao.com/_XXXXXXX/chat`

### 2. 오픈채팅 주소 받기

1. 카카오톡 앱 → **오픈채팅** 탭 → 방 만들기.
2. 만든 방의 설정에서 **링크 복사**.
   `https://open.kakao.com/o/XXXXXXX` 형태다.
3. `docs/config.js` 의 `KAKAO_OPENCHAT_URL` 에 넣는다.
...
window.HG_CONFIG = {
  ...
  KAKAO_CHANNEL_URL:  'https://pf.kakao.com/_XXXXXXX/chat',
  KAKAO_OPENCHAT_URL: 'https://open.kakao.com/o/XXXXXXX',
};
```

따옴표를 빼먹지 않도록 주의한다. 저장하고 커밋·푸시하면 배포된 사이트에도 반영된다.

### 채널과 오픈채팅의 차이

| | 카카오톡 채널 | 오픈채팅 |
| --- | --- | --- |
| 고객이 남기는 정보 | 카카오 계정 (프로필 보임) | 익명 닉네임 |
| 문턱 | 채널 추가를 권유받는다 | 바로 입장 |
| 나중에 알림 보내기 | 가능 (알림톡·메시지) | 불가 |
| 기록 관리 | 관리자홈에서 상담 이력 관리 | 채팅방 안에서만 |

둘을 같이 두는 이유: 정식 문의는 채널로 받아 이력을 남기고, "일단 물어보고 싶은" 고객은
오픈채팅으로 부담 없이 들어오게 하는 것이다.
